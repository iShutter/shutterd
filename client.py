#!/usr/bin/python
# -*- coding: utf-8 -*-
import sys import paho.mqtt.client as mqtt import RPi.GPIO as GPIO 
import time GPIO.setwarnings(False) GPIO.setmode(GPIO.BCM) GPIO.setup(4, 
GPIO.OUT) GPIO.setup(17, GPIO.OUT) GPIO.setup(27, GPIO.OUT) 
GPIO.output(4, 0) GPIO.output(17, 0) GPIO.output(27, 0) class DevNull:
    def write(self, msg):
...
def on_message(client, userdata, msg):
    ...
    if msgs == "on":
        print("Trigger message received. Status: on")
        GPIO.output(4,1)
    else:
        print("Trigger message received. Status: off")
        GPIO.output(4,0) client = mqtt.Client() client.on_connect = 
on_connect client.on_message = on_message client.connect("10.77.96.202", 
1883)
